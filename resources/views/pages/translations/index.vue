...
<template>
    ...
    <Head :title="trans('Translations')" />

    <LayoutDashboard>
        <div class="mx-auto max-w-7xl px-6 py-10 lg:px-8">
            <div v-if="sourceTranslation" class="w-full">
                <div class="w-auto">
                    <InputText v-model="searchQuery" :placeholder="trans('Search languages by name or code')" />
                </div>
            </div>
            ...
            <div v-if="sourceTranslation" class="mt-4 overflow-hidden rounded-lg border bg-white shadow">
                <div class="relative z-10 flex w-full divide-x shadow">
                    ...
                    <div class="hidden flex-1 items-center px-4 text-sm text-gray-500 sm:flex">{{ trans('Language name')
                        }}
                    </div>

                    <div
                        class="hidden flex-1 items-center px-4 text-sm text-gray-500 md:flex md:max-w-72 lg:max-w-80 xl:max-w-96">
                        {{ trans('Translation progress') }}</div>

                    <div class="w-full sm:w-14">
                        <Link v-tooltip="trans('Add Language')" :href="route('ltu.translation.create')"
                            class="relative inline-flex h-14 w-full cursor-pointer select-none items-center justify-center px-4 text-sm font-medium tracking-wide text-gray-700 outline-none transition-colors duration-150 ease-out hover:bg-blue-50 hover:text-blue-500 focus:border-blue-50 sm:text-gray-400">
                        <div class="visible flex size-full items-center justify-center leading-none">
                            <span class="mx-auto whitespace-nowrap sm:hidden">{{ trans('Add Language') }}</span>
                    ...
                    <div class="flex h-full">
                        <div class="flex w-full max-w-full">
                            <button
                                v-tooltip="selectedIds.length ? trans('Delete selected') : trans('Select languages to delete')"
                                type="button"
                    ...
                    <ConfirmationDialog size="sm" :show="showDialog">
                        <div class="flex flex-col p-6">
                            ...
                            <span class="mt-2 text-sm text-gray-500">
                                {{ trans('This action cannot be undone, This will permanently delete the selected languages and all of their translations.') }}
                            </span>

                            <div class="mt-4 flex gap-4">
                                <BaseButton variant="secondary" type="button" size="lg" full-width @click="closeDialog">
                                    {{ trans('Cancel') }} </BaseButton>

                                <BaseButton variant="danger" type="button" size="lg" :is-loading="loading" full-width
                                    @click="deleteTranslations"> {{ trans('Delete') }} </BaseButton>
                            </div>
                        </div>
                    </ConfirmationDialog>
                </div>
                ...
                <div class="divide-y">
                        ...
                        <div class="flex flex-1 divide-x border-r no-underline">
                            ...
                            <div
                                ...
                                <span class="mr-1 shrink-0 text-sm leading-tight text-gray-600">{{
                                    sourceTranslation.phrases_count }} {{ trans('source keys') }}</span>
                            </div>
                            </Link>
                        </div>

                        <Link v-tooltip="trans('Manage Keys')" :href="route('ltu.source_translation')"
                            class="group hidden w-full border-r sm:flex sm:w-14">
                        <div
                            class="relative inline-flex h-14 w-full cursor-pointer select-none items-center justify-center px-4 text-sm font-medium tracking-wide text-gray-700 outline-none transition-colors duration-150 ease-out focus:border-blue-50 group-hover:bg-blue-50 group-hover:text-blue-500">
                            <IconCog
                                class="hidden size-5 fill-current text-gray-400 group-hover:text-blue-500 sm:flex" />
                        </div>
                        </Link>

                        <div v-tooltip="trans('Source language cannot be deleted!')" class="flex h-full">
                            <div class="flex w-full max-w-full">
                                <div
                                    class="relative inline-flex size-14 cursor-not-allowed select-none items-center justify-center p-4 text-sm font-medium uppercase tracking-wide text-gray-400 no-underline outline-none transition-colors duration-150 ease-out">
                                    <IconTrash class="size-5" />
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="text-sm font-medium uppercase tracking-wide">{{ trans('Add new language') }}</div>

                    <IconPlus class="size-6" />
                    </Link>

                    <EmptyState v-else class="bg-gray-50 py-32"
                        :title="trans('There are no languages to translate your project to.')"
                        :description="trans('Add the first one and start translating.')">
                        <template #icon>
                        ...
                        <div class="visible flex items-center leading-none">
                            ...
                            <span class="whitespace-nowrap">{{ trans('Add languages') }}</span>
                        </div>
                        </Link>
                    </EmptyState>
                </div>
            ...
            <EmptyState v-else class="mt-4 min-h-[calc(100vh-11rem)] rounded-md border bg-white py-12 shadow"
                :title="trans('Source language has not been imported yet.')"
                :description="trans('To import the source translation, run the following command in your terminal.')">
                <template #icon>
                    <IconEmptyTranslations class="w-20 text-gray-400" />
                </template>
        ...
        <UseClipboard v-slot="{ copy, copied }"
            source="php artisan translation:import">
            <button v-tooltip="copied ? trans('Copied') : trans('Copy')"
                type="button"
                class="h-full border-l p-2 text-gray-400 hover:bg-blue-50 hover:text-blue-500"
                @click="copy()">
