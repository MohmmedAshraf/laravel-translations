    ...
    <div class="flex h-14 flex-row gap-y-2 no-underline transition-colors duration-100 hover:bg-gray-50">
        <div class="relative flex flex-1 divide-x border-r no-underline">
            ...
            <div class="hidden flex-1 flex-wrap content-center items-center gap-1 px-4 md:flex md:max-w-72 lg:max-w-80 xl:max-w-96">
                <div v-tooltip="`${translation.progress}` + ' ' + trans('strings translated')" class="w-full py-2">
            ...
            <ConfirmationDialog size="sm" :show="showDialog">
                <div class="flex flex-col p-6">
                    ...
                    <span class="mt-2 text-sm text-gray-500">
                        {{ trans('This action cannot be undone, This will permanently delete the :Language language and all of its translations.', { Language: translation.language.name }) }}
                    </span>
                    ...
                    <div class="mt-4 flex gap-4">
                        ...
                        <BaseButton variant="danger" type="button" size="lg" :is-loading="loading" full-width
                            @click="deleteTranslation(translation.id)"> {{ trans('Delete') }} </BaseButton>
                    </div>
                </div>
            </ConfirmationDialog>
        </div>
