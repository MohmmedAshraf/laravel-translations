<script setup lang="ts">
    ...
    const download = () => {
        ...
        fetch(route("ltu.translation.download"))
            ...
            .catch((error) => {
                toast.error(trans("Something went wrong, please try again."), {
                    icon: true,
...
<template>
    ...
    <Dialog size="lg">
        <div v-if="canPublish" class="relative flex flex-col items-start justify-center gap-4 px-6 py-4">
            <div class="flex w-full items-start gap-4">
                ...
                <div class="flex-1">
                    <h3 class="text-xl font-semibold leading-6 text-gray-600">
                        {{ trans('Publish Translations') }}
                    </h3>

                    <p class="mt-1 text-sm text-gray-500">
                        {{ trans('Be cautious when publishing, as this action will update your project with all translations and changes made in the database, overwriting project files.') }}
                    </p>
                </div>
            </div>
            ...
            <div class="w-full text-center">
                <Alert v-if="!isProductionEnv" variant="warning" class="text-left">
                    <strong>{{ trans('Warning') }}:</strong> {{ ('This is a Production Environment. Publishing translations will causing issues with version control and other developers, Please use the download button below instead.') }}
                </Alert>
        ...
        <div v-else class="relative flex flex-col items-start justify-center gap-4 px-6 py-4">
            ...
            <div class="w-full text-center">
                <h3 class="text-xl font-semibold leading-6 text-gray-600">
                    {{ trans('What are you trying to do?') }}
                </h3>

                <p class="mt-4 text-sm text-gray-500">
                    {{ trans("Add languages to your project, then come back to publish translations when you're ready.") }}
                </p>
            </div>
        </div>

        <div class="grid grid-cols-1 gap-6 border-t px-6 py-4" :class="{ 'md:grid-cols-2': canPublish }">
            <BaseButton variant="secondary" type="button" size="lg" tabindex="1" @click="close"> {{ trans('Close') }} </BaseButton>
