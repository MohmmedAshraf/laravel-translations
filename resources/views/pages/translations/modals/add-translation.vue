    ...
    <Dialog size="lg">
        <div class="flex items-start justify-between gap-4 border-b px-6 py-4">
            <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center">
                <IconTranslation class="h-10 w-10 text-gray-500" />
            </div>
        ...
        <div class="mt-0 w-full p-6">
            <vSelect v-model="form.languages" label="name" class="rounded-md bg-white" :options="languages" :reduce="(language: Language) => language.id" :selectable="(language: Language) => !form.languages.includes(language.id)" multiple>
                <template #search="{ attributes, events }">
                    <input class="vs__search" :required="!selected" v-bind="attributes" v-on="events" :placeholder="!form.languages.length ? 'Search languages...' : ''" />
                </template>
        ...
        <div class="grid grid-cols-1 gap-6 border-t px-6 py-4 md:grid-cols-2">
            ...
            <BaseButton @click="submit" variant="primary" type="button" size="lg" :disabled="!form.languages.length || form.processing" :is-loading="form.processing"> Add Languages </BaseButton>
        </div>
    </Dialog>
</template>
