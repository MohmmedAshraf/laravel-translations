    ...
    <LayoutDashboard>
        <div class="w-full bg-white shadow">
            <div class="mx-auto flex w-full max-w-7xl items-center justify-between px-6 lg:px-8">
                ...
                <Link v-tooltip="trans('Go back')" :href="route('ltu.phrases.index', translation.id)" class="flex size-10 items-center justify-center rounded-full bg-gray-100 p-1 hover:bg-gray-200">
                    <IconArrowRight class="size-6 text-gray-400" />
                </Link>
            </div>
        </div>
        ...
        <div class="mx-auto max-w-7xl px-6 py-10 lg:px-8">
            <div class="flex w-full flex-col lg:flex-row">
                <div class="relative w-full overflow-hidden rounded-md bg-white shadow ring-2 ring-blue-100">
                    <div class="flex items-center justify-between border-b">
                        ...
                        <div class="flex items-center divide-x border-l">
                            <button v-tooltip="trans('Copy')" type="button" class="group h-full p-3 hover:bg-blue-50" @click="useTranslation(phrase.source.value)">
                                <IconClipboard class="size-5 text-gray-400 group-hover:text-blue-600" />
                            </button>

                            <Link v-tooltip="trans('Edit Source Key')" :href="route('ltu.source_translation.edit', phrase.source.uuid)" type="button" class="group h-full p-3 hover:bg-blue-50">
                                <IconPencil class="size-5 text-gray-400 group-hover:text-blue-600" />
                            </Link>

                            <button v-tooltip="trans('Delete')" type="button" class="group h-full p-3 hover:bg-red-50">
                ...
                <div class="w-full overflow-hidden rounded-md bg-white shadow ring-2 ring-blue-100 focus-within:ring-blue-400">
                    ...
                    <div class="grid grid-cols-2 border-t border-blue-200">
                        <Link :href="route('ltu.phrases.index', translation.id)" class="flex items-center justify-center bg-blue-50 py-4 text-sm font-medium uppercase text-blue-600 hover:bg-blue-100"> {{ trans('Cancel') }} </Link>
                        ...
                        <BaseButton :is-loading="form.processing" variant="primary" class="items-center rounded-none !py-4 !text-sm !font-medium uppercase" @click="submit">
                            <span class="flex w-auto">{{ trans('Save') }}</span>

                            <span class="hidden md:flex">{{ trans('Translation') }}</span>
                        </BaseButton>
                    </div>
                </div>
            </div>
            ...
            <tabs>
                <tab :name="trans('Suggestions')" class="w-full"
                ...
                <tab :name="trans('Versions')"
                prefix='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M13 3c-4.97 0-9 4.03-9 9H1l3.89 3.89.07.14L9 12H6c0-3.87 3.13-7 7-7s7 3.13 7 7-3.13 7-7 7c-1.93 0-3.68-.79-4.94-2.06l-1.42 1.42C8.27 19.99 10.51 21 13 21c4.97 0 9-4.03 9-9s-4.03-9-9-9zm-1 5v5l4.25 2.52.77-1.28-3.52-2.09V8z"/></svg>'>
                    <div class="flex h-48 w-full items-center justify-center px-4 py-6">
                        <span class="text-sm text-gray-500">{{ trans('Coming soon...') }}</span>
                    </div>
                </tab>
            </tabs>
        </div>
