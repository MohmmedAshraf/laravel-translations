...
<template>
    ...
    <Head :title="trans('Contributors')" />

    <LayoutDashboard>
        <div class="mx-auto max-w-7xl px-6 lg:px-8">
            <tabs>
                <tab :name="trans('Contributors')"
                    ...
                    <div class="w-full divide-y overflow-hidden rounded-md bg-white shadow">
                            ...
                            <div class="flex h-14 w-full divide-x">
                                <div class="grid w-full grid-cols-2 divide-x md:grid-cols-3">
                                    <div class="flex w-full items-center justify-start px-4">
                                        <span class="text-sm font-medium text-gray-400">{{ trans('Name') }}</span>
                                    </div>
                                ...
                                <div class="grid w-16">
                                    <Link v-tooltip="trans('Invite Contributor')"
                                        :href="route('ltu.contributors.invite')" href="#"
                        ...
                        <div v-for="contributor in contributors.data" :key="contributor.id"
                            ...
                            <div class="flex h-14 w-full divide-x">
                                ...
                                <div class="grid w-16">
                                    <button v-if="user.id === contributor.id"
                                        v-tooltip="trans('You cannot delete yourself!')" disabled
                                        class="group flex cursor-not-allowed items-center justify-center bg-gray-50 px-3">
                                        <IconTrash class="size-5 text-gray-400" />
                                    </button>

                                    <button v-else v-tooltip="trans('Delete')"
                                        class="group flex items-center justify-center px-3 hover:bg-red-50"
                                        @click="openDialog">
                                ...
                                <ConfirmationDialog size="sm" :show="showDialog">
                                    <div class="flex flex-col p-6">
                                        ...
                                        <span class="mt-2 text-sm text-gray-500">
                                            {{ trans('This action cannot be undone, This will permanently delete the :Name invitation.', { Name: contributor.name }) }}
                                        </span>

                                        <div class="mt-4 flex gap-4">
                                            <BaseButton variant="secondary" type="button" size="lg" full-width
                                                @click="closeDialog"> {{ trans('Cancel') }} </BaseButton>

                                            <BaseButton variant="danger" type="button" size="lg" :is-loading="loading"
                                                full-width @click="deleteContributor(contributor.id)">
                                                {{ trans('Delete') }}
                                            </BaseButton>
                                        </div>
                                    </div>
