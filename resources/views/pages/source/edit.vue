...
const submit = () => {
    form.post(route('ltu.translation.source_language.update', props.phrase.uuid), {
        onSuccess: () => {
            form.reset();
        },
    });
    ...
    <LayoutDashboard>
        <div class="w-full bg-white shadow">
            <div class="mx-auto flex w-full max-w-7xl items-center justify-between px-6 lg:px-8">
                ...
                <Link v-tooltip="'Go back'" :href="route('ltu.translation.source_language')" class="flex h-10 w-10 items-center justify-center rounded-full bg-gray-100 p-1 hover:bg-gray-200">
                    <IconArrowRight class="h-6 w-6 text-gray-400" />
                </Link>
            </div>
        </div>
            ...
            <div class="flex w-full flex-col lg:flex-row">
                <div class="relative w-full overflow-hidden rounded-md bg-white shadow ring-2 ring-blue-100">
                    ...
                    <div class="flex h-[calc(100%-80px)]">
                        <input-textarea id="textArea" v-model="form.updatePhrase" rows="7" autofocus class="h-full w-full resize-none rounded-none border-none px-4 py-2.5 shadow-none ring-transparent focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-0" />
                    </div>

                    <div class="grid grid-cols-2 border-t border-blue-200">
                        <span class="px-4 py-2.5 text-xs text-gray-400">Characters Length: {{ form.updatePhrase.length }}</span>
                    </div>
                </div>
                ...
                <div class="w-full overflow-hidden rounded-md bg-white shadow ring-2 ring-blue-100 focus-within:ring-blue-400">
                    <div class="flex min-h-[231px] px-4 py-2">
                        {{ phrase?.group }}.{{ phrase?.key }}
                    </div>

                    <div class="grid grid-cols-2 border-t border-blue-200">
                        <Link :href="route('ltu.translation.source_language')" class="flex items-center justify-center bg-blue-50 py-4 text-sm font-medium uppercase text-blue-600 hover:bg-blue-100">
                            Cancel
                        </Link>
