...
<template>
    ...
    <Head :title="trans('Base Language')" />
    ...
    <LayoutDashboard>
        <div class="w-full bg-white shadow">
            <div class="mx-auto flex w-full max-w-7xl items-center justify-between px-6 lg:px-8">
                ...
                <Link v-tooltip="trans('Go back')" :href="route('ltu.translation.index')"
                ...
                <div class="flex w-full flex-wrap items-center justify-between gap-4 px-4 py-3 sm:flex-nowrap">
                    ...
                    <div class="w-full max-w-full md:max-w-sm">
                        <InputNativeSelect id="translationFile" v-model="phraseTranslationFile" size="md"
                            :placeholder="trans('Filter by file')" :items="translationFiles" />
                    </div>
                </div>
                ...
                <div class="w-full shadow-md">
                    <div class="flex h-14 w-full divide-x">
                        ...
                        <div class="grid w-full grid-cols-2 divide-x">
                            ...
                            <div class="flex w-full items-center justify-start px-4">
                                <span class="text-sm font-medium text-gray-400">{{ trans('String') }}</span>
                            </div>
                        </div>

                        <div class="grid w-36 grid-cols-2 divide-x">
                            <Link v-tooltip="trans('Add New Key')"
                                :href="route('ltu.source_translation.add_source_key')"
                            ...
                            <div class="flex h-full">
                                <div class="flex w-full max-w-full">
                                    <button
                                        v-tooltip="selectedIds.length ? trans('Delete selected') : trans('Select phrases to delete')"
                                        type="button"
                            ...
                            <ConfirmationDialog size="sm" :show="showDialog">
                                <div class="flex flex-col p-6">
                                    ...
                                    <span class="mt-2 text-sm text-gray-500">
                                        {{ trans('This action cannot be undone, This will permanently delete the selected phrases and all of their translations.') }}
                                    </span>
                                    ...
                                    <div class="mt-4 flex gap-4">
                                        ...
                                        <BaseButton variant="danger" type="button" size="lg" :is-loading="loading"
                                            full-width @click="deletePhrases"> {{ trans('Delete') }} </BaseButton>
                                    </div>
                                </div>
                            </ConfirmationDialog>
                        </div>
